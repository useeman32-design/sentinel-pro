    ...
    return `
      <div class="grid grid-4">
        <div class="card stat-card" style="--stat-glow:rgba(0,255,136,.12)">
          ...
          <div class="stat-value" style="color:var(--green)">84<span style="font-size:16px;color:var(--text-dim)">/100</span></div>
        ...
        <div class="card stat-card" style="--stat-glow:rgba(255,176,32,.12);animation-delay:.15s">
          ...
          <span class="stat-delta up">Improved from Medium</span>
        </div>
      </div>
      ...
      <div class="section-gap grid grid-2">
        <div class="card">
          ...
          ${Charts.line(scoreHistory, { color: '#00FF88' })}
        </div>
        <div class="card">
          <div class="card-title">${Icons.clock} Login History</div>
          ${Charts.line(logins, { color: '#00C8FF' })}
        </div>
      </div>
      ...
      <div class="section-gap grid grid-main">
        ...
        <div style="display:flex;flex-direction:column;gap:16px">
          <div class="card">
            <div class="card-title">${Icons.info} Security Tips</div>
            ${[
              'Enable two-step verification on WhatsApp today — it takes 30 seconds.',
              'Your bank will NEVER ask for your full BVN, PIN, or OTP. Ever.',
              'Length beats complexity: a 14-character passphrase outlasts P@ssw0rd! by centuries.',
            ].map((t, i) => `
              <div class="list-item">
                <div class="list-icon ${['green', 'blue', 'amber'][i]}">${Icons.check}</div>
                <div class="list-body"><div class="list-sub" style="white-space:normal;color:var(--text)">${t}</div></div>
              </div>`).join('')}
          </div>
          <div class="card">
            <div class="card-title">${Icons.globe} Latest Threat News<span class="spacer"></span><a class="card-link" href="#/threat-intel">More</a></div>
            ${[
              ['danger', 'Fake CBN "account upgrade" SMS wave hits 12k+ users', '2h ago'],
              ['warn', 'WhatsApp hijack via fake voting links spreading', '6h ago'],
              ['info', 'New Android banking trojan variant detected', '2d ago'],
            ].map(([lvl, t, time]) => `
              <div class="list-item">
                <span class="pill ${lvl}" style="flex:none"><span class="pdot"></span></span>
                <div class="list-body"><div class="list-sub" style="white-space:normal;color:var(--text)">${t}</div><div class="list-sub">${time}</div></div>
              </div>`).join('')}
          </div>
        </div>
      </div>`;
